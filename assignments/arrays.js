...
// PROJECT RESTRICTION: You can't use map, reduce, or filter to solve these
// problems.  Only use native JavaScript for loops.
...
// ==== Challenge 1 ====
// The dealer can't recall the information for a car with an id of 33 on his
// lot. Help the dealer find out which car has an id of 33 by logging the car's
// year, make, and model in the console log provided to you below:

function getCar(inventory, carId) {
    for (i = 0; i < inventory.length; i++) {
        if (inventory[i].id == carId) {
            let car = inventory[i];
            return `Car ${carId.toString()} is a ${car.car_year.toString()} ${
                car.car_make
            } ${car.car_model}.`;
        }
    }
...
console.log(getCar(inventory, 33));
