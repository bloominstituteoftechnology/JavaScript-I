...
let car33stretch = inventory.filter(x => x['id'] === 33);
console.log(`Car 33 is a ${car33['car_year']} ${car33['car_make']} ${car33['car_model']}`);
...
let carModelsStretch = inventory.map(x => x['car_model']).sort()
...
let carYearsStretch = inventory.map(x => x['car_year']);
...
let OldCarsStretch = inventory.filter(x => x['car_year'] < 2000);
// ==== Challenge 6 ====
