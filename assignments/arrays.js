...
// Stretch Solution for Challenge 4

let stretchCarYears = inventory.map((year) => {
...
// Stretch Solution for Challenge 5

let stretchOldCars = inventory.filter((year) => {
    if (year.car_year < 2000) return year.car_year;
}).map((year) => {
    return year.car_year;
})
...
// Stretch Solution for Challenge 6

let stretchBMWAudi = inventory.filter((fancy) => {
    return fancy.car_make === "BMW" || fancy.car_make === "Audi";
})
