...
let carModels = [];
inventory.sort();
carModels.push(inventory);
console.log("Car models inventory sorted: ",carModels);
console.log("*****************");
...
let oldCars =[];
for(i = 0; i < inventory.length; i++) {
	newCars = inventory[i].car_year;
	if(newCars > 2000) {
		console.log("New car: " + inventory[i].car_year);
		console.log("*****************");

	} else if (newCars < 2000) {
		olderCars = newCars;
		console.log("Old car: " + olderCars);
...
for (var key in inventory) {
	if( inventory[key].car_make === "BMW" || inventory[key].car_make === "Audi") {
		bmwAndAudi = inventory[key];
		// console.log(JSON.stringify(bmwAndAudi));
		BMWAndAudi.push(bmwAndAudi);
		JSON.stringify(key, inventory[key]);
	}
}
