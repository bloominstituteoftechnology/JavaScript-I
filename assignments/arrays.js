...
// I did this the hard way first since I thought we couldn't use .sort()... sigh...
    // const order = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ'
    // let sortArr = function() {
    //     for(let i = 0; i < order.length; i++) {
    //         for(let idx = 0; idx < inventory.length; idx++) {
    //             let car_model = inventory[idx]['car_model'];

    //             if (car_model[0].includes(order[i])) {
    //                 carModels.push(car_model);
    //             }
    //         }
    //     }
    //     return carModels;
...
console.log(sortModel(inventory));

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];
const sortYear = function() {
    for (let i = 0; i < inventory.length; i++) {
        carYears.push(inventory[i]['car_year']);
    }
    return carYears.sort();
}
console.log(sortYear(inventory));
