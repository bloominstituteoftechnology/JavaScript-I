...
// Function to find any car by it's id
function findCar(n){
...
let lastCar = 0;
console.log(`${inventory[inventory.length-1].car_make} ${inventory[inventory.length-1].car_model} is the last car in the inventory.`);
...
let carYears = [];
function findCarYears(){
  for (let i = 0; i < inventory.length; i++){
    carYears.push(inventory[i].car_year);
  }
  return carYears;
}
console.log(findCarYears());
