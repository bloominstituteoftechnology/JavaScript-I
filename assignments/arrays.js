...
for (let i = 32; i < 33; i+=32){
  get33.push(inventory[i]);
};

for(let i = 0; i<get33.length; i++){
  console.log(`Car 33 is a ${get33[i].car_year} ${get33[i].car_make} ${get33[i].car_model}`);
}
...
let lastCar = inventory.slice(-1).pop();
...
console.log("These are all the BMW and  Audi cars in our inventory" + JSON.stringify(BMWAndAudi));
