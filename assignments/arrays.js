...
// changed this to const, so the datatype of inventory cannot be overwritten on accident
const inventory = [{
        "id": 1,
        "car_make": "Lincoln",
        "car_model": "Navigator",
...
const searchID = (idNum) => {
    // for (let i = 0; i < inventory.length; i++) {
    //     if (inventory[i].id === idNum) {
    //         console.log(`Car ${i+1} is a ${inventory[i].car_year} ${inventory[i].car_make} ${inventory[i].car_model}` );
    //     }
    // }
...
//         console.log(`Last Car, number ${i+1} ${inventory[i].car_make} ${inventory[i].car_model}`);
//     }
// }
inventory.forEach((x, i) => {
    if (i === inventory.length - 1){
        console.log(`Last Car, number ${x.id}, ${x.car_make} ${x.car_model}`)
    }
})
// ==== Challenge 3 ====
...
const carModels = inventory.map(x => x.car_model);
...
const carYears = inventory.map(x => x.car_year);
console.log(`${carYears}`);
...
// let BMWAndAudi = [];
// for (let i = inventory.length - 1; i >= 0; i = i - 1) {
//     let x = inventory[i]
...
const isBMWorAudi = (current) => {
    if(current.car_make === "BMW" || current.car_make === "Audi") {
        return current;
}
}
const BMWAndAudi = inventory.filter(isBMWorAudi);
console.log(JSON.stringify(BMWAndAudi));