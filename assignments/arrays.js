...
console.log(`Car 33 is a ${inventory[34].car_year} ${inventory[32].car_make} ${inventory[32].car_model} ` );
...
const challenge3 = getFields(inventory, "car_model");
...
let carYears = [];
console.log(getFields(inventory,"car_year"));

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.

function numberFields(input, field) {
    const output = [];
    for (var i=0; i < input.length ; ++i)
...
const challenge5 = numberFields(inventory, "car_year")
...
function audi (input, field) {
    let bmw = [];
    for (let i=0; i < input.length ; ++i)
    if(input[i][field] === "BMW" || input[i][field] ===  "Audi"){
        bmw.push(input[i])};
    return bmw;
}

const challenge6 = audi(inventory,"car_make")

challenge6v2 = JSON.stringify(challenge6);
console.log(challenge6v2)