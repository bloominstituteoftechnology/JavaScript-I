...
for (let i= 0; i < inventory.length; i++) {
    if (inventory[i].id === 33) { // inventory is equal to 33
        console.log('Car 33 is a ' + inventory[i].car_year + ' ' + inventory[i].car_make + ' ' +inventory[i].car_model + '.' ); // print this
    }
}
...
console.log();
for (let i=0; i <= inventory.length; i++) {
    if (inventory [i] === inventory[inventory.length - 1]) // once i = inventory length - 1 (last inventory item)
           console.log('Last model is' + ' ' + inventory[i].car_make + ' ' + inventory[i].car_model +'.'); // print this
     }
...
let carModels = []; //reference carModels with array assigned
for (let i=0; i < inventory.length; i++) {
    carModels.push(inventory[i]['car_model']) // adds new items to end of array
}
carModels.sort(); // sorts array items
console.log(carModels); // print
...
for(let i = 0; i < inventory.length; i++){ 
    carYears.push(inventory[i]['car_year']); // adds new items to end of array
}
...
for (let i = 0; i < inventory.length; i++) {
    if (inventory[i] < 2000) {
        oldCars.push(carYears[i]); // adds new items to end of array
    }
    }
    console.log(oldCars);
