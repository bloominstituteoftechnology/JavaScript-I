...
let carModels = [];
let carModelsSorted = inventory.map(function(i){
  return i.car_model;
}).sort();
...
let carYears = inventory.map(function(i){
  return i.car_year;
});
...
let BMWAndAudi = inventory.filter(function(i){
  return i.car_make==="BMW"|| i.car_make==="Audi" }
);
