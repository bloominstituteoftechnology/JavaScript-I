...
for (let i = 0; i < inventory.length; i++) {
  if (inventory[i].id === 33) {
    const car33_year = inventory[i].car_year;
    const car33_make = inventory[i].car_make;
    const car33_model = inventory[i].car_model;
    console.log(`Car 33 is a ${car33_year} ${car33_make} ${car33_model}.`);
  }
};
...
for (let i = lastCar; i < inventory.length; i++) {
  if (i === inventory.length-1) {
    const carlast_make = inventory[i].car_make;
    const carlast_model = inventory[i].car_model;
    console.log(`The last car in the inventory is a ${carlast_make} ${carlast_model}.`);
  }
};
...
let carJSON = JSON.stringify(BMW_Audi(inventory, BMWAndAudi));
