...
let carYears = [];
carYears = inventory.sort(function(a, b){
    carYears = a.car_year - b.car_year;
    return carYears;
})
...
for(let x = 0; x<carYears.length; x++){
    if(carYears[x].car_year < 2000){
       oldCars.push(carYears[x].car_year)
    }
    else if(carYears[x] > 2000){
        return 0;
    }
}
...
for(let i=0; i< inventory.length; i++){
    ...
    if(inventory[i].car_make.toUpperCase() === "BMW"){
        BMWAndAudi.push(inventory[i]);
    }
