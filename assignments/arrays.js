...
for (let n = 0; n < inventory.length; n++) {
    carModels[n] = inventory[n].car_model;
}
carModels.sort();
...
for (let k = 0; k < inventory.length; k++) {
    if (2000 >= inventory[k].car_year) {
        oldCars[oldCarsIndex] = inventory[k].car_make + " " + inventory[k].car_model;
        oldCarsIndex++;
    }
}
...
for(let j = 0; j < inventory.length; j++) {
    if(inventory[j].car_make === "Audi" || inventory[j].car_make === "BMW") {
        BMWAndAudi[BNAindex] = inventory[j];
        BNAindex++;
    }
}

console.log(JSON.stringify(BMWAndAudi)); //NOTE: not sure if this is what the challenge is asking for
