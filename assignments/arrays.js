...
for (let i in inventory) {
  if (inventory[i].id === 33)  //look for object with inventory id of 33
  console.log(`Car 33 is a ${inventory[i].car_year} ${inventory[i].car_make} ${inventory[i].car_model}` ); //logs a string with requested information
...
let lastCar = 0;
while (lastCar < (inventory.length - 1)) {
  lastCar++; //iterate through inventory to find index of last item
}
console.log(`The last car in our inventory is a ${inventory[lastCar].car_make} ${inventory[lastCar].car_model}`);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let carModels = [];
for (let i in inventory) {
  carModels.push(inventory[i].car_model); //iterate through inventory, and push all car_model entries into carModels array
}
for (let i in carModels) {
  if (carModels[i].charAt(0) === carModels[i].charAt(0).toLowerCase()) { //check for uncapitalized carModel values
    carModels[i] = carModels[i].charAt(0).toUpperCase() + carModels[i].slice(1);  //capitalize any uncapitalized carModel values, for sorting purposes
  }
}
carModels.sort();  //alphabetize carModels array values
...
let carYears = [];
for (let i in inventory) {
  carYears.push(inventory[i].car_year); //iterate through inventory and push all year entries into carYears array
}
...
for (let i in carYears) {
  if (carYears[i] < 2000) oldCars.push(carYears[i]); //iterate through carYears array and add push entries less than 2000 into oldCars array
}
console.log(`There are ${oldCars.length} cars older than the year 2000`); //log number of cars older than 2000 using length property of oldCars array

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi =[];
for (let i in inventory) {
  if (inventory[i].car_make === "BMW" || inventory[i].car_make === "Audi") { // iterate through inventory and check car_make entries for BMW or Audi
    BMWAndAudi.push(inventory[i]); //push any matching entries into BMWAndAudi array
  }
}
