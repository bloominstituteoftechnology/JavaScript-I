...
function loopToLast (thisArray) {
    let lastCarInArray = undefined;
    for (var i in thisArray){
        lastCarInArray = thisArray[i]; 
    }
...
function lessThanArray (thisArray, targetValue) {
    let tempArray = [];
    for (var i in thisArray){
        if (thisArray[i] < targetValue){
            tempArray.push(thisArray[i]);
        }
    }
    return tempArray;
}

function searchSpecificMakes (thisArray, make1, make2){
    let tempArray = [];
    for (var i in thisArray){
        if (thisArray[i].car_make == make1 || thisArray[i].car_make == make2){
            tempArray.push(thisArray[i]);
        }
    }
...
let findCarByID = param => inventory.filter(function (inv) {return inv.id == param});
let idCar = findCarByID(33)[0];
console.log(`Car ${idCar.id} is a ${idCar.car_year} ${idCar.car_make} ${idCar.car_model}.`); 


// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
//let lastCar = 0;
//lastCar = loopToLast(inventory);
//console.log(`The last car in inventory is a ${lastCar.car_make} ${lastCar.car_model}.`);
let lastCar = inventory[inventory.length - 1];
console.log(`The last car in inventory is a ${lastCar.car_year} ${lastCar.car_make} ${lastCar.car_model}.`)
...
console.log(inventory.map(function (e){return e.car_model}).sort());
...
console.log(inventory.map(function (e){return e.car_year}));

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year
// 2000 by populating the array oldCars and logging it's length.
//let oldCars =[];
//oldCars = lessThanArray(carYears, 2000);
//console.log(oldCars.length); 
console.log(inventory.filter(function (e){return parseInt(e.car_year) < 2000}).length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi
// array, use JSON.stringify() to show the results of the array in the console.
//let BMWAndAudi =[];
//BMWAndAudi = searchSpecificMakes(inventory, "BMW", "Audi");
//console.log(JSON.stringify(BMWAndAudi));
console.log(inventory.filter(function (e){if (e.car_make == "BMW" || e.car_make == "Audi"){return e}}));
