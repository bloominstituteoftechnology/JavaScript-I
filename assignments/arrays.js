...
function getInventoryCar(carId) {
  ...
  return "car ID: " + carId + " not found";
}
console.log(`Car 33 is a ` + getInventoryCar(33).car_year + ' ' + getInventoryCar(33).car_make + ' ' + getInventoryCar(33).car_model);

//// Stretch use map()
inventory.map(function (car){
  if (car.id === 33) {
    console.log(`(stretch map()): Car 33 is a ` + car.car_year.toString() + ' ' + car.car_make + ' ' + car.car_model);
  }
});
...
console.log("Last car make and model: " + inventory[lastCar].car_make + ' ' + inventory[lastCar].car_model);
...
//// Stretch use filter()
let BMWAndAudiStretch = inventory.filter(car => car.car_make === "BMW" || car.car_make === "Audi");
console.log("Stretch filter(): \n" + JSON.stringify(BMWAndAudiStretch));