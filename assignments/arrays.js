...
let oldCars=[];
 for(let i=0; i <inventory.length; i++){ if(inventory[i].car_year < 2000) {
   
    oldCars[i]=(inventory[i].car_year);
}};

console.log(oldCars);
console.log(Object.keys(oldCars).length);
...
for (i=0; i<inventory.length; i++) {if(`${inventory[i].car_make}` === "Audi" || `${inventory[i].car_make}`==="BMW"){
