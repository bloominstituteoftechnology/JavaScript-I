...
// let's not assume the ids are all in order with no gaps
let carId = 33;
let carEntry = null;
for (let i = 0; i < inventory.length; i++) {
    carEntry = inventory[i];
    if (carEntry.id === carId) {
        break;
    }
}

console.log(`Car ${carId} is a ${carEntry.car_year} ${carEntry.car_make} ${carEntry.car_model}` );


// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and
// model of the last car in the inventory?  Log the make and model into the console.
...
let carModels = [];
// get the car models--not clear from specs if repeats should be included
for (let i = 0; i < inventory.length; i++) {
    carModels.push(inventory[i].car_model);
}
// sort the list of car models
carModels.sort();
// log them to the console, one per line
console.log('\nThe car models are:');
...
console.log(`\nThere are ${oldCars.length} cars older than model year 2000`);
...
console.log('\nThe BMW and Audi models in inventory are:');
