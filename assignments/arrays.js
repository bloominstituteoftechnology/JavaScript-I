...
const car_index = 33 - 1;
console.log(`Car 33 is a ${inventory[car_index].car_year} ${inventory[car_index].car_make} ${inventory[car_index].car_model}\n` );
...
for (let i = 0; i < inventory.length; i++) {
  ...
  for (let j = 0; j < carModels.length; j++) {
    if (carModels[j] == inventory[i].car_model) new_model = false;
  }
  if (new_model) carModels.push(inventory[i].car_model);
}
...
console.log(`We have ${oldCars.length} cars older than 2000.\n`);
