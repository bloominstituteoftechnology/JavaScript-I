...
for (let i=0; i < inventory.length; i++) {
  if (inventory[i].id === 33) {
    ...
    console.log(`Car 33 is a ${carYear} ${carMake} ${carModel}`);  // => "Car 33 is a 2011 Jeep Wrangler"
  }
}
...
for (let i=0; i < inventory.length; i++) {
  ...
  if (carMake === 'BMW' || carMake === 'Audi') {
    BMWAndAudi.push(carModel);
  }
}

console.log(JSON.stringify(BMWAndAudi));  // => "["riolet","4000CS Quattro","525","6 Series","Q7","TT"]"
