...
for (let model in inventory){
    carModels.push(inventory[model]['car_model']);
}
...
for (let each in inventory){
    if ((inventory[each]['car_make'] === "BMW") || (inventory[each]['car_make'] === "Audi")){
    BMWAndAudi.push(inventory[each])
}
