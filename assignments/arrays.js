...
let car = inventory.find(c => c.id === 33);
if (car) console.log(`Car 33 is a ${car.car_year} ${car.car_make} ${car.car_model}`);
...
for (let car of inventory) {
    if (car.car_year) carYears.push(car.car_year);
}
...
console.log("Cars older then 2000: ", oldCars.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];
for (let car of inventory) {
    if (car.car_make.toLowerCase() === "bmw" || car.car_make.toLowerCase() === "audi") BMWAndAudi.push(car);
}

console.log("All BMWs and Audis: ", JSON.stringify(BMWAndAudi));
