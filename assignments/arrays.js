...
console.log(`The last car is car ${inventory[lastCar].id} and is a ${inventory[lastCar].car_make} ${inventory[lastCar].car_model}.`);
...
carYears = carYears.sort().reverse(); // added so that the years are in order and so that the most recent appear first
...
let oldCars =[];
for (let car in inventory) {
    currentCar = inventory[car].car_year;
    if (currentCar < 2000) {
        oldCars.push(inventory[car]);
    }
}
