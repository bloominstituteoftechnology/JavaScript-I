...
getCarByIndex = (index) => {
    let car = inventory[index]
    return `make: ${car.car_make} model: ${car.car_model}`;
}

console.log(getCarByIndex(lastCar));
...
getCarsProperty = (property) =>{
    if(inventory[0].hasOwnProperty(property)){
        let properties = [];
        for(let index in inventory){
            properties.push(inventory[index][property]);
        }
        return properties;
    }
    return 'Does not exist'
}
let carModels = getCarsProperty('car_model');
...
let carYears = getCarsProperty('car_year');
console.log(carYears);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let oldCars = carYears.sort().slice(0,carYears.indexOf(2000));
console.log(oldCars.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.

getCarsByPropertyAndValue = (property, value) => {
    let cars = [];
    for(let index in inventory){
        if(inventory[index][property] === value){
...
let BMWAndAudi =[];
BMWAndAudi.push(getCarsByPropertyAndValue('car_make','BMW'));
BMWAndAudi.push(getCarsByPropertyAndValue('car_make', 'Audi'))
