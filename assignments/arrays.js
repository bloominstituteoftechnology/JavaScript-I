...
console.log(inventory[i=inventory.length-1].car_make + inventory[i=inventory.length-1].car_model);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
let sortedbymake = inventory.sort(function(a,b){
    var texta = a.car_make.toUpperCase();
    var textb = b.car_make.toUpperCase();
...
function oldcar (year) {
    return year < 2000;
  }
  
  let oldcarsfiltered = carYears.filter(oldcar);
  
  console.log(oldcarsfiltered);
// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.

var bmwOrAudi = inventory.filter(inventory=>inventory.car_make=="BMW" ||inventory.car_make=="Audi");

console.log(JSON.stringify(bmwOrAudi));

let bmworaudi = [];
 
for (var i = 0 ; i<inventory.length; i++){
    if (inventory[i].car_make == "BMW" || inventory[i].car_make =="Audi"){
    bmworaudi.push(inventory[i]);
    }
  }
