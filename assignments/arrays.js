...
let inventory = [
  ...
  inventory.forEach(obj => {
    if (obj.id === 33) {
      console.log(
        `Car ${obj.id} is a ${obj.car_year} ${obj.car_make}  ${obj.car_model}`
      );
    }
  });
  ...
  for (let i = 0; i < carYears.length; i++) {
    if (inventory[i].car_year < 2000) oldCars.push(i);
  }
  ...
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_make == 'BMW' || inventory[i].car_make == 'Audi')
      BeamAndAudi.push(inventory[i]);
  }
  console.log(JSON.stringify(BeamAndAudi));