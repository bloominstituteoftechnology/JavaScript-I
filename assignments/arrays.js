...
for (let c3 = 0; c3 < inventory.length; c3++){
    carModels.push(inventory[c3].car_model);
}
carModels.sort();
...
for (let c4 = 0; c4 < inventory.length; c4++){
	carYears.push(inventory[c4]["car_year"])
	}
...
for (let c5 = 0; c5 < inventory.length; c5++){
	if (carYears[c5] <= 2000) {
		oldCars.push(inventory[c5]["car_year"])
	}
}
...
for (let c6 = 0; c6 < inventory.length; c6++){
    if(inventory[c6]["car_make"] === "BMW" || inventory[c6]["car_make"] === "Audi") {
        BMWAndAudi.push(inventory[c6]);
    }
}
