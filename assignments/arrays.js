...
let lastCar = 0;
console.log(inventory[inventory.length - 1].car_make , inventory[inventory.length - 1].model);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console
const alphaCar = () => {
    const betaCars = [];
    betaCar.push (inventory[0]);

    for (let i = 1; betaCars.length <= inventory.length;i++) {
        for (let j = 1; betaCars.length <= inventory.length;i++)
    }
}
...
for (let i = 0; carYears.length < inventory.length + 1; i++)
