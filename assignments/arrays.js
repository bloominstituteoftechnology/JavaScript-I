...
let carModels = [];
function mySort () {
    // below creates duplicate array, as to not sort in place 'inventory'
    carModels = inventory.concat();
    // sorting array below
...
let oldCars =[];
// console.log();
function myOldCarYears () {
    inventory.forEach((inventory) => {
        if (inventory.car_year < 2000) {
        oldCars.push(inventory.car_year)
        } 
    });
    console.log(`\"(1999 & older) Cars in Inventory: ${oldCars.length}\"`);
} 
myOldCarYears();
...
let BMWAndAudi =[];
function germanJunk (arr1, arr2){
    for (let i = 0; i < inventory.length; i++) {
        if (inventory[i].car_make === arr1 || inventory[i].car_make === arr2) {
