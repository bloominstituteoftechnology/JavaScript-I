...
let idNum = 0;
function runThis(lotNumber){
  for(let i = 0; i < inventory.length; i++){
    if (inventory[i].id === lotNumber) {
      idNum = i;
      return;
    }
  }
...
runThis(33);
console.log(`Car 33 is a ${inventory[idNum].car_year} ${inventory[idNum].car_make} ${inventory[idNum].car_model}`);
...
function collectModels(list){
  let placeholder = [0]
  for(let i=0; i < list.length; i++){
    placeholder[i] = list[i].car_model;
  }
  return placeholder
}
let carModels = collectModels(inventory);
let carModelsSorted = carModels.sort();
console.log(carModelsSorted);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
function collectYears(list){
  let placeholder = [0]
  for(let i=0; i < list.length; i++){
    placeholder[i] = list[i].car_year;
  }
  return placeholder
}
...
let oldCars = olderThan(carYears, 2000);
console.log(oldCars.length);

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory. Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
function brandFilter(brand1, brand2){
  let placeholder = [0];
  for(let i=0; i < inventory.length; i++){
    if(inventory[i].car_make === brand1){
      placeholder.push(inventory[i]);
    }
    else if(inventory[i].car_make === brand2){
      placeholder.push(inventory[i]);
    }
  }
  placeholder.shift();
...
let BMWAndAudi = brandFilter("BMW", "Audi");
