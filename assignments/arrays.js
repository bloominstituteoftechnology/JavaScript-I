...
// ================ SIMPLE SOLUTION: =====================================================
// note: no need to iterate through array -> car objects aleady ordered -> we can simply use car id - 1
// console.log(`Car 33 is a ` + inventory[32].car_year + ` ` + inventory[32].car_make + ` ` + inventory[32].car_model);
// ================ BETTER SOLUTION: =====================================================
// note: we can make a function that takes both the id and the car array and returns the information

const returnCarInfo = (id, cars) => {
  const carIndex = id - 1
  // edge case if carIndex out of bounds
  if (carIndex > cars[cars.length - 1]) return 'id out of bounds'
  const car = cars[carIndex]
  return (
    `Car 33 is a ` + car.car_year + ` ` + car.car_make + ` ` + car.car_model
  )
}
console.log(returnCarInfo(33, inventory))

// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.

// including car_year -> looks better that way -> this would be important to a used-car sales person
const returnLastCarInfo = cars => {
...
console.log(returnLastCarInfo(inventory))

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console

const returnModelsAlphabetically = cars => {
  const modelArr = []
  cars.forEach(car => {
    modelArr.push(car.car_model)
  })
  modelArr.sort();
  return modelArr;
}
const sortedAlphabetically = returnModelsAlphabetically(inventory)
console.log("Here are the car models, listed alphabetically: ")
...
const years = returnCarYears(inventory);
years.sort()
console.log(years);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
const returnCarsBeforeYear2000 = () => {
  const oldCars = []
  years.forEach((year) => {
    if (year < '2000') {
      oldCars.push(year);
...
const carsBeforeYear2000 = returnCarsBeforeYear2000()
console.log("cars before year 2000: ", carsBeforeYear2000)


// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory. Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.

const returnAudiOrBMW = (cars) => {
  const luxury = []
  cars.forEach((car) => {
...
const only = JSON.stringify(returnAudiOrBMW(inventory))
console.log(only)