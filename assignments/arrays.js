...
// The dealer can't recall the information for a car with an id of 33 on his lot. Help the dealer find out which car has an id of 33 by logging the car's year, make, and model in the console log provided to you below:
{
	...
	inventory.sort((a,b) => (a.id > b.id) ? 1 : ((b.id < a.id) ? -1 : 0));  //lets make sure the id is in order.
	let mycar = 0;
	console.log("Car " + mycar.id+ " is a " + mycar.car_year + " " + mycar.car_make + " " + mycar.car_model);
  ...
  //Didn't want to trust that the data would always be equal to the id number (consider if they're are duplicates or skipped numbers).
...
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
{
	...
	let mycar = (inventory.length)? inventory[0]: 0;
      for (let i =0; i < inventory.length;i++){
		if((inventory[i])["id"] > mycar.id) {
          mycar=inventory[i];
		}
	}
	console.log("Car " + mycar.id+ " is a " + mycar.car_year + " " + mycar.car_make + " " + mycar.car_model);
	// "Last" is ambiguious..did you mean the car with the highest ID or the last one in the array (those things aren't equiv).
...
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
{
	...
	let carYears = [];
	inventory.forEach((el, i) => (carYears.push(el.car_year)));
	console.log(carYears.sort().filter((e,i,a) => i==a.indexOf(e)));
// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
	console.log("Challenge 5");
	const oldCarDef = 2000;
	let oldCars = carYears.filter((e,i,a) => (e < oldCarDef));
...
{
	let BMWAndAudi = inventory.filter((e,i,a) => (e.car_make.toUpperCase() === "BMW" || e.car_make.toUpperCase() === "AUDI")?1:0);
