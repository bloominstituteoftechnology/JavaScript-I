...
function getModels(item) {
    let carModel = item.car_model;
    return carModel;
}

let carModels = inventory.map(getModels);
...
function getYears(item) {
    let carYear = item.car_year;
    return carYear;
}

let carYears = inventory.map(getYears);
console.log(carYears);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.

function getOldCars(item) {
...
let oldCarsOne = inventory.map(getOldCars);
let oldCarsTwo = oldCarsOne.filter(function (item) {return item != null;});
console.log(oldCarsTwo.length); 

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.

function getBMWAndAudi(item) {
...
let BMWAndAudi1 = inventory.map(getBMWAndAudi);
let BMWAndAudi2 = BMWAndAudi1.filter(function (item) {return item != null;});
console.log(BMWAndAudi2);
