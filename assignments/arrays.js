...
inventorySorted = inventory.sort(dynamicSort("car_model"));
...
function olderThanTwoThousand(input, field) {
    var oldCars =[];
    for (i = 0; i < input.length; i++) {
      if (input[i][field] < 2000) {
        oldCars.push(input[i][field]);
      }
    }
    return oldCars.length;
};
  
  console.log(olderThanTwoThousand(inventory, "car_year"));

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
function BMWandAudi(input, field) {
    var BMWandAudiOnly =[];
    for (i = 0; i < input.length; i++) {
      if (input[i][field] === "BMW") {
        BMWandAudiOnly.push(input[i]);
      } else if (input[i][field] === "Audi") {
        BMWandAudiOnly.push(input[i]);
      }
    }
    return BMWandAudiOnly;
  }
  
  console.log(BMWandAudi(inventory, "car_make"));