...
let car = {};
for(let i = 0; i < inventory.length; i++) {if(inventory[i].id === 33) car = inventory[i];}
...
let lastCar = {};
for(let i = 0; i < inventory.length; i++) {if(i === inventory.length - 1) lastCar = inventory[i];}
...
// let carModels = [];
// function compareObjectProperties(x, y) { return x.car_model.toLowerCase() < y.car_model.toLowerCase() ? -1 : 1;}
// inventory.sort(compareObjectProperties).forEach(function(car) {carModels.push(car.car_model);});
// console.log(carModels);
function bubble(array) {
    for(let i = 0; i < array.length - 1; i++){
        for(let j = 0; j < array.length - i - 1; j++) {
            if(array[j].car_model.toLowerCase() > array[j + 1].car_model.toLowerCase()) {
                let temp = array[j];
                array[j] = array[j+1];
                array[j+1] = temp;
            }
...
let carModels = bubble(inventory);
console.log(carModels);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];
// inventory.forEach(function(car) {carYears.push(car.car_year);});
...
let oldCars =[];
// carYears.forEach(function(year) {if(year < 2000) oldCars.push(year);});
for(let i = 0; i < carYears.length; i++) {if(inventory[i].car_year < 2000) oldCars.push(carYears[i]);}
console.log(oldCars.length); 

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi =[];
// inventory.forEach(function(car) {if(car.car_make === "BMW" || car.car_make === "Audi") BMWAndAudi.push(car);});
for(let i=0; i < inventory.length; i++) {if(inventory[i].car_make === "BMW" || inventory[i].car_make === "Audi") BMWAndAudi.push(inventory[i]);}
